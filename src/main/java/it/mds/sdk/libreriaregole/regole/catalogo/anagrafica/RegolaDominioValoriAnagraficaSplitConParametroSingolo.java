...
@Slf4j
@NoArgsConstructor
@XmlDiscriminatorValue("regolaDominioValoriAnagraficaSplitConParametroSingolo")
public class RegolaDominioValoriAnagraficaSplitConParametroSingolo extends RegolaGenerica{

	public RegolaDominioValoriAnagraficaSplitConParametroSingolo(String nome, String codErrore, String desErrore, Parametri parametri) {
	...
	public List<Esito> valida(String nomeCampo, RecordDtoGenerico recordDtoGenerico) {
		...
		try {
			...
			int parametroLunghezzaSplitListaDominio = Integer.parseInt(this.getParametri().getParametriMap().get("parametroLunghezzaSplitListaDominio"));
			...
			listaDominio = listaDominio.stream().map(ex->{
				String[] element = ex.split("#");
				return element.length == parametroLunghezzaSplitListaDominio ? element[parametroSplitter] : "";
			}).collect(Collectors.toList());
