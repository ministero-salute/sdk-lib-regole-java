...
@Slf4j
@NoArgsConstructor
@XmlDiscriminatorValue("regolaNoImplicitoNoEsplicitoMaggioreZero")
public class RegolaNoImplicitoNoEsplicitoMaggioreZero extends RegolaGenerica {

    public RegolaNoImplicitoNoEsplicitoMaggioreZero(String nome, String codErrore, String desErrore, Parametri parametri) {
     ...
     * (vedi definizione codice 1 sheet BR - Definizioni) allora il campo da Validare non può essere composto da soli 0
     *
    ...
    public List<Esito> valida(String nomeCampo, RecordDtoGenerico recordDtoGenerico) {
        ...
        try {
            ...
            for (int i = 0; i < campoDaValidare.length(); i++) {
                if (campoDaValidare.charAt(i) == '0') {
                    counter++;
                }
            }

            if (!esitoAnonimatoImplicito && !esitoAnonimatoEsplicito && counter == campoDaValidare.length()) {
                listaEsiti.add(creaEsitoKO(nomeCampo, this.getCodErrore(), this.getDesErrore()));
            } else {
            ...
            throw new ValidazioneImpossibileException("Impossibile validare la RegolaNoImplicitoNoEsplicitoMaggioreZero per il campo " + nomeCampo);
        }
        return listaEsiti;
    }
}
