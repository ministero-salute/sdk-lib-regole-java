...
class RegolaDominioValoriAnagraficaSplitConParametroTest {
    @Mock
    ...
    RegolaDominioValoriAnagraficaSplitConParametro regola;
    ...
    void init() {
        ...
        parametri.put("primoParametro", "annoRiferimento");
        parametri.put("secondoParametro", "codiceRegione");
        parametri.put("terzoParametro", "codiceAziendaSanitariaRiferimento");
        parametriTest = new Parametri();
        parametriTest.setParametriMap(parametri);

        regola = spy(new RegolaDominioValoriAnagraficaSplitConParametro("TEST", "TEST", "TEST", parametriTest));
    }

    @Test
    void costruttoreVuoto() {
        RegolaDominioValoriAnagraficaSplitConParametro regola = new RegolaDominioValoriAnagraficaSplitConParametro();
        assertTrue(regola instanceof RegolaDominioValoriAnagraficaSplitConParametro);
    }
    ...
    void validaOK() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("campoDaValidare")).thenReturn("ADD");
        Mockito.when(recordMockito.getCampo("annoRiferimento")).thenReturn("2022");
        Mockito.when(recordMockito.getCampo("codiceRegione")).thenReturn("080");
        Mockito.when(recordMockito.getCampo("codiceAziendaSanitariaRiferimento")).thenReturn("030");
    ...
    @Test
    void valida1dateNull() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {

        List<RecordAnagrafica> recordAnagraficas = new ArrayList<>();
        recordAnagraficas.add(new RecordAnagrafica(null, null, "2022#080#030#ADD"));
    ...
    void validaKO() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        List<RecordAnagrafica> recordAnagraficas = new ArrayList<>();
        recordAnagraficas.add(new RecordAnagrafica(LocalDate.of(1901, 1, 1).atStartOfDay(), LocalDate.of(2100, 1, 1).atStartOfDay(), "2022#080#030#AD"));
        recordAnagraficas.add(new RecordAnagrafica(LocalDate.of(1901, 1, 1).atStartOfDay(), LocalDate.of(2100, 1, 1).atStartOfDay(), "mock#mock"));
