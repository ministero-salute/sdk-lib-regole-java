...
class RegolaDominioValoriAnagraficaFacoltativo2Test {
    @Mock
    ...
    RegolaDominioValoriAnagraficaFacoltativo2 regola;
    ...
    void init() {
        ...
        regola = spy(new RegolaDominioValoriAnagraficaFacoltativo2("TEST", "TEST", "TEST", parametriTest));
    }

    @Test
    void costruttoreVuoto() {
        RegolaDominioValoriAnagraficaFacoltativo2 regola = new RegolaDominioValoriAnagraficaFacoltativo2();
        assertTrue(regola instanceof RegolaDominioValoriAnagraficaFacoltativo2);
    }
    ...
    void validaKO() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("campoDaValidare")).thenReturn("smc");
    ...
    @Test
    void validaKO_DateNull() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {

        List<RecordAnagrafica> recordAnagraficas = new ArrayList<>();
        recordAnagraficas.add(new RecordAnagrafica(null, null, "smcb"));
    ...
    void validaOk1() throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, MalformedRegistryException, RegistryNotFoundException {
        ...
        Mockito.when(recordMockito.getCampo("campoDaValidare")).thenReturn("smcb");
        Mockito.when(regola.getGestoreAnagrafica()).thenReturn(gestoreAnagrafica);
        doReturn(tabellaAnagrafica).when(gestoreAnagrafica).richiediAnagrafica(any(), any(), anyBoolean());
        Mockito.when(tabellaAnagrafica.getRecordsAnagrafica()).thenReturn(recordAnagraficas);
